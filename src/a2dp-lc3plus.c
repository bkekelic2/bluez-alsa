	...
	for (ba_transport_pcm_state_set_running(t_pcm);;) {
		...
		if (lc3plus_frames > 0) {
			...
			for (;;) {
				...
				warn("LC3plus payload fragmentation: extra %zu bytes", payload_len);
...
void *a2dp_lc3plus_dec_thread(struct ba_transport_pcm *t_pcm) {
	...
	for (ba_transport_pcm_state_set_running(t_pcm);;) {
		...
		if (missing_pcm_frames > 0) {
			...
			warn("Missing LC3plus frames: %zu", missing_lc3plus_frames);
		}
#endif
		...
		if (rtp_media_header->fragmented &&
			...
			warn("Resizing LC3plus payload buffer: %zd -> %zd", bt_payload.nmemb, len_);
		...
		if (rtp_media_header->fragmented &&
			...
			warn("Fragmented LC3plus frame [%u]: payload len: %zd",
...
static int a2dp_lc3plus_configuration_check(
	...
	case LC3PLUS_FRAME_DURATION_100:
		break;
	default:
		warn("LC3plus: Invalid frame duration: %#x", conf->frame_duration);
		return A2DP_CHECK_ERR_FRAME_DURATION;
	}

	if (a2dp_bit_mapping_lookup(a2dp_lc3plus_channels, conf_v.channel_mode) == 0) {
		warn("LC3plus: Invalid channel mode: %#x", conf->channel_mode);
	...
	if (a2dp_bit_mapping_lookup(a2dp_lc3plus_samplings, conf_sampling_freq) == 0) {
		warn("LC3plus: Invalid sampling frequency: %#x", A2DP_LC3PLUS_GET_SAMPLING_FREQ(*conf));
