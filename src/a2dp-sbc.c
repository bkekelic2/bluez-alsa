...
static int a2dp_sbc_configuration_check(
	...
	if (a2dp_bit_mapping_lookup(a2dp_sbc_samplings, conf_v.sampling_freq) == 0) {
		warn("SBC: Invalid sampling frequency: %#x", conf->sampling_freq);
		return A2DP_CHECK_ERR_SAMPLING;
	}

	if (a2dp_bit_mapping_lookup(a2dp_sbc_channels, conf_v.channel_mode) == 0) {
		warn("SBC: Invalid channel mode: %#x", conf->channel_mode);
	...
	case SBC_BLOCK_LENGTH_16:
		break;
	default:
		warn("SBC: Invalid block length: %#x", conf->block_length);
	...
	case SBC_SUBBANDS_8:
		break;
	default:
		warn("SBC: Invalid sub-bands: %#x", conf->subbands);
	...
	case SBC_ALLOCATION_LOUDNESS:
		break;
	default:
		warn("SBC: Invalid allocation method: %#x", conf->allocation_method);
	...
	warn("SBC: Selected bit-pool range: [%u, %u]",
			conf->min_bitpool, conf->max_bitpool);
...
static int a2dp_sbc_source_init(struct a2dp_sep *sep) {
	...
	if (config.sbc_quality == SBC_QUALITY_XQ ||
			config.sbc_quality == SBC_QUALITY_XQPLUS) {
		warn("SBC: Activating SBC Dual Channel HD (SBC %s)",
